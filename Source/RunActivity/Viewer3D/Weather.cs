...
namespace Orts.Viewer3D
{
    public class WeatherControl
    {
        ...
        public virtual void Update(ElapsedTime elapsedTime)
        {
            ...
            {
                // Multiplayer weather has changed so we need to update our state to match weather, overcastFactor, precipitationIntensity and fogDistance.
                ...
                if (MPManager.Instance().precipitationIntensity >= 0) { Weather.PrecipitationIntensityPPSPM2 = MPManager.Instance().precipitationIntensity; UpdateVolume(); }
                ...
                try
                {
                    if ((MPManager.Instance().weather >= 0 && MPManager.Instance().weather != (int)Viewer.Simulator.WeatherType) || MPManager.Instance().overcastFactor >= 0 || MPManager.Instance().precipitationIntensity >= 0 || MPManager.Instance().fogDistance >= 0)
                    {
                        MPManager.Instance().weatherChanged = false;
                        MPManager.Instance().weather = -1;
                        MPManager.Instance().overcastFactor = -1;
                        MPManager.Instance().precipitationIntensity = -1;
            ...
            // If we're a multiplayer server, send out the new overcastFactor, precipitationIntensity and fogDistance to all clients.
            if (MPManager.IsServer())
            {
